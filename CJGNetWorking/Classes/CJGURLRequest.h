...
@class CJGUploadData;
@interface CJGURLRequest : NSObject
NS_ASSUME_NONNULL_BEGIN

#pragma mark - 配置请求
///用于标识不同类型的request状态   默认为CJGRequestTypeRefresh 不读取缓存，不存储缓存
@property (nonatomic,assign) CJGApiType apiType;


 ///用于标识不同的请求类型
 ///默认请求类型为 GET请求
 ///可在公共配置  setupBaseConfig方法内   更改默认请求类型config.methodType=
@property (nonatomic,assign) CJGMethodType methodType;

 ///下载请求操作状态
@property (nonatomic,assign) CJGDownloadState  downloadState;

///请求参数的类型
///单次请求设置 请求格式 默认JSON，优先级大于 公共配置，不影响其他请求设置
@property (nonatomic,assign) CJGRequestSerializerType requestSerializer;

///响应数据的类型
///单次请求设置 响应格式 默认JSON，优先级大于 公共配置，不影响其他请求设置
@property (nonatomic,assign) CJGResponseSerializerType responseSerializer;

///请求的服务器地址，例如。"https://github.com/Suzhibin/"，默认为nil。
///server 优先级大于公共配置 baseServer
@property (nonatomic,copy) NSString *server;

///请求的接口路径，例如。"CJGNetworking/issues"，默认为nil。
@property (nonatomic,copy) NSString *path;

///请求的最终URL，由' server '和' path '属性组合而成 例如："https://github.com/Suzhibin/CJGNetworking/issues"  默认为' nil
...
///为上传请求提供数据
@property (nonatomic,strong,nullable) NSMutableArray<CJGUploadData *> *uploadDatas;
#if TARGET_OS_IPHONE
@property (nonatomic, weak, readonly, nullable) id<CJGURLRequestDelegate> delegate;
#elif TARGET_OS_MAC
@property (nonatomic, assign, readonly, nullable) id<CJGURLRequestDelegate> delegate;
#endif
@property (nonatomic, copy, readonly, nullable) CJGRequestSuccessBlock successBlock;

@property (nonatomic, copy, readonly, nullable) CJGRequestFailureBlock failureBlock;

@property (nonatomic, copy, readonly, nullable) CJGRequestFinishedBlock finishedBlock;

@property (nonatomic, copy, readonly, nullable) CJGRequestProgressBlock progressBlock;
...
@interface CJGBatchRequest : NSObject

///请求url 列队容器
@property (nonatomic, strong , nullable) NSMutableArray<CJGURLRequest *> * requestArray;
...
///上传文件数据的类
@interface CJGUploadData : NSObject
