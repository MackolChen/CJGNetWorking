...
@property (nonatomic, assign) CJGRequestSerializerType baseRequestSerializer;
@property (nonatomic, assign) CJGResponseSerializerType baseResponseSerializer;
@property (nonatomic, assign) CJGMethodType baseMethodType;
@property (nonatomic, assign) BOOL consoleLog;
@property (nonatomic, strong) NSSet <NSString *> *baseHTTPMethodsEncodingParametersInURI;
@end

@implementation CJGRequestEngine{
    NSMutableDictionary * _requestDic;
}

+ (instancetype)defaultEngine{
    static CJGRequestEngine *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[CJGRequestEngine alloc]init];
    });
...
- (NSUInteger)dataTaskWithMethod:(CJGURLRequest *)request
...
#pragma mark - upload
- (NSUInteger)uploadWithRequest:(CJGURLRequest *)request
    ...
    NSURLSessionDataTask *uploadTask = [self POST:[NSString cjg_stringUTF8Encoding:request.url] parameters:request.parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        
        [request.uploadDatas enumerateObjectsUsingBlock:^(CJGUploadData *obj, NSUInteger idx, BOOL *stop) {
            if (obj.fileData) {
...
- (NSUInteger)downloadWithRequest:(CJGURLRequest *)request resumeData:(NSData *)resumeData savePath:(NSString *)savePath progress:(void (^)(NSProgress *downloadProgress)) downloadProgressBlock completionHandler:(void (^)(NSURLResponse *response, NSURL *filePath, NSError *error))completionHandler{
    ...
    NSURLRequest *urlRequest = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString cjg_stringUTF8Encoding:request.url]]];
...
//请求参数的格式
- (void)requestSerializerConfig:(CJGURLRequest *)request{
    self.requestSerializer =request.requestSerializer==CJGHTTPRequestSerializer ?[AFHTTPRequestSerializer serializer]:[AFJSONRequestSerializer serializer];
...
//请求头设置
- (void)headersConfig:(CJGURLRequest *)request{
...
- (void)configBaseWithRequest:(CJGURLRequest *)request progressBlock:(CJGRequestProgressBlock)progressBlock successBlock:(CJGRequestSuccessBlock)successBlock failureBlock:(CJGRequestFailureBlock)failureBlock finishedBlock:(CJGRequestFinishedBlock)finishedBlock target:(id<CJGURLRequestDelegate>)target{
    ...
    if (request.methodType==CJGMethodTypeUpload) {
        request.apiType=CJGRequestTypeKeepFirst;
    }
    if (request.methodType==CJGMethodTypeDownLoad) {
        request.apiType=CJGRequestTypeKeepFirst;
    }
    //=====================================================
...
- (void)printParameterWithRequest:(CJGURLRequest *)request{
    if (request.consoleLog==YES) {
        NSString *requestStr=request.requestSerializer==CJGHTTPRequestSerializer ?@"HTTP":@"JOSN";
        NSString *responseStr=request.responseSerializer==CJGHTTPResponseSerializer ?@"HTTP":@"JOSN";
        NSLog(@"\n------------CJGNetworking------request info------begin------\n-URLAddress-: %@ \n-parameters-:%@ \n-Header-: %@\n-userInfo-: %@\n-timeout-:%.2f\n-requestSerializer-:%@\n-responseSerializer-:%@\n------------CJGNetworking------request info-------end-------",request.url,request.parameters, self.requestSerializer.HTTPRequestHeaders,request.userInfo,self.requestSerializer.timeoutInterval,requestStr,responseStr);
    }
}
